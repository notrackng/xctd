...
- **The app tracks and validates payments against last week, not the real calendar week — this is a genuine business-logic policy, not just a display tweak.** `WeeklyObligationService::operatingNow(DateTimeImmutable $realNow)` (`$realNow->modify('-7 days')`) is the single place this one-week offset lives; every caller that means "the week we're actually tracking against" — `sync()`, `canAcceptPayment()`, `paymentEligibility()` (in `index.php`, `api/sender-options.php`, `api/transactions.php`), and `WeeklyObligationPresenter::presentForDisplay()` (via `dashboard(operatingNow($now))`) — routes real "now" through it first rather than passing `$now` directly. A receipt uploaded during the real current week therefore settles *last* week's obligation. `presentForDisplay()` also surfaces the real, still-open current week separately as `incoming_label`, purely informational (no obligation row is created for it until `operatingNow()` catches up to it next week). Because of this, last week's obligation rows must stay `unpaid`/`pending` — marking one `paid` with no backing `payment_transaction_id` (e.g. an ad hoc admin correction) removes the slot a genuine upload needs to land in and will make `canAcceptPayment()` wrongly refuse that sender's next receipt. If this policy ever needs to change, `operatingNow()` is the only place to touch — never shift `$now` ad hoc at an individual call site, or the validation path and the display path will disagree with each other. `SummaryRepository`'s "week" summary card (the dashboard's top total/count tile) is deliberately *not* part of this policy — it totals the real current week, unrelated to `operatingNow()`. A `SummaryRepository`-vs-`WeeklyObligationService` week mismatch is intentional, not a bug: don't "fix" one to match the other without being asked.
- **A brand-new sender whose `tracking_start_week` is after the operating week is a real edge case in the above, not a corner to skip.** `dashboard()`'s row-building loop excludes such a sender from `rows`/counters entirely (they'd otherwise show a misleading `pending` pill for a week they were never tracked in — `COALESCE(cur.status, 'pending')` defaults to `pending` when there's simply no obligation row, which looks identical to "owes and hasn't paid"). But `canAcceptPayment()`/`paymentEligibility()` must *not* read that exclusion as "block them" the way they do for a fully-settled or disabled sender — a sender who is missing from `dashboard()`'s rows purely because their own tracking hasn't reached the operating week yet must still be allowed to make their first payment, or they can never pay until next week's cycle catches up to them. `WeeklyObligationService::trackingNotYetStarted()` is the shared check both methods use to tell this case apart from the others; if you touch either method's "not found" fallback, keep routing it through that helper.
